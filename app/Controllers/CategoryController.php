...
class CategoryController extends Controller {
    ...
    {
        $alias = $params["cat_alias"];
        $alias = htmlspecialchars($alias);

        $check = $this->DB->exec("SELECT * FROM `categories` WHERE `alias` = '$alias';");
        if(!$check)
        {
        ...
        //SEO
        $f3->set('page_title', $cat_name . ' - всі новини в Україні');
        $f3->set('description_seo', ' Всі новини від українських ЗМІ в категорії ' . $cat_name);
        ...
        $news['last'] = $this->DB->exec(
            "SELECT news.*, partners.name AS source_name FROM `news` INNER JOIN `partners` ON partners.id = news.source WHERE news.category_id = $cat_id $justGoodQuery $allowedQuery ORDER BY news.id DESC LIMIT 25");
